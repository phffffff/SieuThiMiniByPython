...
class ProductGUI:
    def __init__(self):
        biz = ProductBiz()
        self.dulieu = biz.get_all_product()
        self.result = []
        print(self.dulieu)
        for i in self.dulieu:
            self.result.append(list(i))
        self.Headings = ['ID', 'Tên Sản phẩm', 'số lượng', 'Giá tiền', 'Mã giảm giá', 'Tên loại sản phẩm']
        ...
        layout1=  [[sg.Text('Chọn từ khóa search:',size=15),sg.Combo(['ID','NAME','COUNT','PRICE','DISCOUNT','PRODUCTTYPEID'], key='-KEY-'),sg.Text('Content:'),sg.Input(key='-CONTENT-',size=22),sg.Button('SEARCH')],
                      ...
                      [sg.Text('Giảm giá:',size=15), sg.Input(key=self.Headings[4])],
