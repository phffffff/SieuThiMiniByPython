...
class MembershipsGui:
    def __init__(self):
        ...
        self.Headings = [' ID ', 'VERIFICATION CODE', 'NAME', 'BIRTHDAY', 'PHONE', 'Mail', 'Point']

        self.lstMemberships = MembershipsBiz().get_all_memberships(cond={"is_active": 1})
        self.result=[]
        ...
        layout1=  [[sg.Text('Chọn từ khóa search:',size=15),sg.Combo(['id','verification_code','name','birthday','phone','mail','point'], default_value="id", key='-COMBO_SEARCH-',enable_events=True),sg.Text('Content:'),sg.Input(key='-CONTENT-',size=22,enable_events=True)],
    ...
    def run(self):
        ...
        while True:
            ...
            if event == "Exit" or event == sg.WINDOW_CLOSED:
                ...
                data = {"id":id[2:],"verification_code": verification_code, "name": name, "birthday": birthday, "phone": phone, "mail": mail, "point": point,"is_active": 1}
                ...
                upd = MembershipsBiz().update_memberships(product=data, cond={"id": id[2:]})
                if upd != -1:
