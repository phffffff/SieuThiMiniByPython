...
class AccountDAL(Crud_Dal):
    def __init__(self):
        self.conn = Conn()
        Crud_Dal.__init__(self, tableName="accounts", conn=self.conn)

    def list_data_with_cond(self, fields="*", cond=None, key_order_by=None, limit=None):
        return super().listDataWithJson(fields=fields, where=cond, order_by=key_order_by, limit=limit)

    def find_data_with_cond(self,fields = "*", conditions=None, key_order_by=None, limit=None):
        return super().findDataWithJson(fields=fields,where=conditions, order_by=key_order_by, limit=limit)
